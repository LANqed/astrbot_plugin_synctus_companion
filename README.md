...
装完后私聊发 `Synctus 状态`：能看到活动、电量、待办和「Synctus 上报：已连接」
就说明整条链路通了。若显示「未连接」，`Synctus 连接` 会给出具体原因。

## 命令

```
Synctus 状态     现在在做什么、手机电量、今日专注、待办进度、上报状态
Synctus 日程     今天的完整日程（标出当前段与日程来源）
Synctus 待办     待办清单与临近的重要日期
Synctus 电量     手机电量与预计可用时间
Synctus 连接     Synctus 上报状态与房间标识前缀
Synctus 静音     暂停主动消息（只影响发命令的人）
Synctus 恢复     恢复主动消息
```
